...
export const fetchIssues = () => (dispatch, getState) => {
  ...
  FetchService(
    ...
    if (res.length > 0) {
      if (isLastResultPage) {
        dispatch(toggleResultListEnd());
      } // response may have more results, enable next button

      dispatch(syncResultFromAPI(res)); // if response has results, sync

      if (res.length < 30) {
        dispatch(toggleResultListEnd());
      } // issuesPerpage is 30, so if result has less than 30, then disable next button
    } else {
      dispatch(toggleResultListEnd());
    } // if response has no results, disable next button
  });
};
